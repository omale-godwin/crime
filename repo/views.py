...
def evident(request):
    ...
    if request.method == 'POST' and request.FILES['myevident']:
         ...
         evident_data = Evident(photo_main=photo_main, dates=dats, user_id=user_id, uuid=uuid.uuid4())
         evident_data.save()
         ReportEv = Report.objects.get(id=request.session['id'])
         ReportEv.user_report = evident_data
         ReportEv.save()
...
def weakness(request):
    if request.method == 'POST':  
        ...
        data_weakness = Weakness(user_id=user_id, name=w_name, regnumber=w_regnumber, dats=w_dats, email=w_email, phone=w_phone, description=w_description, uuid=uuid.uuid4())
        data_weakness.save()
        ReportWe = Report.objects.get(id=request.session['id'])
        ReportWe.user_weakness = data_weakness
        ReportWe.save()
